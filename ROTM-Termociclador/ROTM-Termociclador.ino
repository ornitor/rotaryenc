...
#define ST_CYCLING  3
#define ST_STABLING  31
#define ST_OVESHOOTING  32
#define ST_HALTED  4
#define ST_ENDED  5


char *mainMenu[5] = { "Retorna", "Prog","Ciclo","Estabiliza","Overshoot"};
...
void setup() 
{
      displayInit("Termociclador BH", "Bom dia!");
      Serial.begin(115200);
      delay(200);
      //Serial.println("\r\n\r\nTermociclador BH");
...
void loop()
{
      if(gstate == ST_STANDBY){
          show("Termociclador BH");
          if (debounce(pinClick)){
              gstate = ST_MENU;                       
          }
      } else if(gstate == ST_MENU){
          menu("Termociclador BH", mainMenu , 5);
          if (debounce(pinClick)){
              if( posEncoder%5 == 1)
         ...
         posEncoder++;
      } else if (gstate == ST_STABLING){ 
          // nao tem menu, inputs fim do processo ou click
          if(iCiclo == 0 ) lcd.clear();
          if ( stabilizing()){   // retorna true quando terminar a ciclagem
              gstate = ST_ENDED;  
              setEnded();
          }
          else if (debounce(pinClick)){
         ...
         posEncoder++;
      } else if (gstate == ST_OVESHOOTING){ 
          // nao tem menu, inputs fim do processo ou click
          if(iCiclo == 0 ) lcd.clear();
          if ( overshooting()){   // retorna true quando terminar a ciclagem
              gstate = ST_ENDED;  
              setEnded();
          }
          else if (debounce(pinClick)){
